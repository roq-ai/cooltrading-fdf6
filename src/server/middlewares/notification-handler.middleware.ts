...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order.create.create': { roles: ['owner', 'administrator'], key: 'order', tenantPath: [], userPath: [] },
  'pricing.update.update': { roles: ['owner', 'administrator'], key: 'pricing', tenantPath: [], userPath: [] },
  'policy.update.update': { roles: ['owner', 'administrator'], key: 'policy', tenantPath: [], userPath: [] },
};
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.exchange.findFirst({
