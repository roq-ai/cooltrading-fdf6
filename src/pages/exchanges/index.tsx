...
import { getExchanges, deleteExchangeById } from 'apiSdk/exchanges';
import { ExchangeInterface } from 'interfaces/exchange';

type ColumnType = ColumnDef<ExchangeInterface, unknown>;
...
export function ExchangeListPage(props: ExchangeListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getExchanges({
        relations: ['user', 'market.count', 'transaction.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ExchangeInterface>>(
    () => `/exchanges?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: ExchangeInterface) => {
    if (hasAccess('exchange', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/exchanges/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/exchanges/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('exchange', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/exchanges/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('exchange', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
