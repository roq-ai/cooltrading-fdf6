...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createWalletTransactions();
    default:
  ...
  async function getWalletTransactions() {
    ...
    const response = await prisma.wallet_transactions
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'wallet_transactions'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createWalletTransactions() {
    ...
    const data = await prisma.wallet_transactions.create({
      data: body,
    });
