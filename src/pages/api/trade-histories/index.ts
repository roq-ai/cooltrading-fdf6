...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getTradeHistories();
    case 'POST':
      return createTradeHistory();
    default:
  ...
  async function getTradeHistories() {
    ...
    const response = await prisma.trade_history
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'trade_history'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createTradeHistory() {
    ...
    const data = await prisma.trade_history.create({
      data: body,
    });
