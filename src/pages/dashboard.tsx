...
import { getMarketTrends } from 'apiSdk/market-trends';
import { MarketTrendsInterface } from 'interfaces/market-trends';
import { getSecuritySettings } from 'apiSdk/security-settings';
import { SecuritySettingsInterface } from 'interfaces/security-settings';
import { getTradeHistories } from 'apiSdk/trade-histories';
import { TradeHistoryInterface } from 'interfaces/trade-history';

import { TradeHistoryListPage as ShowCaseList } from 'pages/trade-histories';

function DashboardPage() {
  const { session } = useSession();

  const marketTrendsStat = useSWR<PaginatedInterface<MarketTrendsInterface>>(
    () => `/market-trends`,
    () => getMarketTrends({ limit: 1, offset: 0 }),
  );

  const securitySettingsStat = useSWR<PaginatedInterface<SecuritySettingsInterface>>(
    () => `/security-settings`,
    () => getSecuritySettings({ limit: 1, offset: 0 }),
  );

  const tradeHistoryStat = useSWR<PaginatedInterface<TradeHistoryInterface>>(
    () => `/trade-histories`,
    () => getTradeHistories({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Market Trends',
      result: marketTrendsStat?.data?.totalCount,
      isLoading: marketTrendsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Security Settings',
      result: securitySettingsStat?.data?.totalCount,
      isLoading: securitySettingsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Trade Histories',
      result: tradeHistoryStat?.data?.totalCount,
      isLoading: tradeHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Trade Histories
                </Text>
                <NextLink href={`trade-histories`} passHref legacyBehavior>
                  <Button
