...
import { getWallets, deleteWalletById } from 'apiSdk/wallets';
import { WalletInterface } from 'interfaces/wallet';

type ColumnType = ColumnDef<WalletInterface, unknown>;
...
export function WalletListPage(props: WalletListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<WalletInterface>>(
    () => `/wallets?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: WalletInterface) => {
    if (hasAccess('wallet', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/wallets/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/wallets/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('wallet', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/wallets/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('wallet', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="wallet" />
        </Flex>

        {hasAccess('wallet', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/wallets/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
