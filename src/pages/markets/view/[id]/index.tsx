...
import { getMarketById } from 'apiSdk/markets';
...
function MarketViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<MarketInterface>(
    () => (id ? `/markets/${id}` : null),
    () =>
      getMarketById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('market', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/markets/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                {hasAccess('currency', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Currency"
                    text={
                      <Link as={NextLink} href={`/currencies/view/${data?.currency?.id}`}>
                        {data?.currency?.name}
                      </Link>
                    }
                  />
