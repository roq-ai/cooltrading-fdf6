...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<ExchangeInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/exchanges` : null),
    () => getExchanges({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Exchanges',
      path: isTenantUser ? `/exchanges/view/${data?.[0]?.id}` : '/exchanges',
      entity: 'exchange',
    ...
    { name: 'Markets', path: '/markets', entity: 'market', service: AccessServiceEnum.PROJECT, icon: FiPieChart },
    {
      name: 'Market Trends',
      path: '/market-trends',
    ...
    { name: 'Wallets', path: '/wallets', entity: 'wallet', service: AccessServiceEnum.PROJECT, icon: FiBox },
    {
